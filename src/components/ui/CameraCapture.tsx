...
// Add OpenCV types
type OpenCV = any; // Use a generic type for OpenCV to avoid type conflicts

// Extend Window interface to include Face Detection API and OpenCV
declare global {
  interface Window { 
    FaceDetector?: {
...
export default function CameraCapture({
  ...
  // Initialize OpenCV and face classifier
  useEffect(() => {
    const initializeOpenCV = async () => {
      try {
        ...
        (window as any).cv = cv; // Use type assertion to avoid type conflicts
        
        // Load face classifier
        faceClassifierRef.current = new cv.CascadeClassifier();
        await faceClassifierRef.current.load('haarcascade_frontalface_default.xml');
        
        // Load gender model
        const tf = await import('@tensorflow/tfjs');
        genderModelRef.current = await tf.loadLayersModel('/gender_model/model.json');
        
        setIsOpenCVLoaded(true);
        console.log('OpenCV and models initialized successfully');
      } catch (error) {
  ...
  const detectFacesWithAPI = async () => {
    ...
    try {
      ...
      // Detect gender if face is found and OpenCV is loaded
      if (faceFound && isOpenCVLoaded && window.cv && genderModelRef.current) {
        const canvas = canvasRef.current;
        const ctx = canvas.getContext('2d', { willReadFrequently: true });
        if (ctx) {
          ...
          const opencvFaces = new window.cv.RectVector();
          faceClassifierRef.current.detectMultiScale(gray, opencvFaces);
          
          // Process detected faces
          for (let i = 0; i < opencvFaces.size(); i++) {
            const face = opencvFaces.get(i);
            const roi = gray.roi(face);
            
            // Gender detection
            const tensor = await import('@tensorflow/tfjs');
            const genderTensor = tensor.browser.fromPixels(roi)
              .resizeNearestNeighbor([224, 224])
              .toFloat()
              .expandDims();
            
            const prediction = await genderModelRef.current.predict(genderTensor).data();
            const gender = prediction[0] > 0.5 ? 'Male' : 'Female';
            setDetectedGender(gender);
            
            // Draw rectangle and gender text
            ...
            window.cv.putText(
              src,
              gender,
              new window.cv.Point(face.x, face.y - 10),
          ...
          <div className={`absolute inset-0 pointer-events-none border-4 ${faceDetected ? (gestureDetected ? 'border-green-500' : 'border-yellow-500') : 'border-dashed border-white/30'} m-8 rounded-full transition-colors duration-300`}>
            {faceDetected && (
              <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-3 py-1 rounded-full text-xs font-medium">
                Face Detected {detectedGender ? `(${detectedGender})` : ''}
              </div>
            )}
